...
public class GQLProductServices {
    ...
    public static Map<String, Object> searchProductsByGoodIdentificationValue(DispatchContext dctx,
        ...
        List<EntityExpr> exprs = UtilMisc.toList(EntityCondition.makeCondition(
                ...
                EntityOperator.AND, EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("idValue"),
                        EntityOperator.LIKE, EntityFunction.UPPER(("%" + idFragment + "%").toUpperCase()))));

        EntityFindOptions options = new EntityFindOptions();
        String orderBy = null;
        if (context.containsKey("pagination")) {
            PaginationInputType pagination = (PaginationInputType) context.get("pagination");
            options.setLimit(pagination.getPageSize());
            options.setMaxRows(pagination.getPageSize());
            options.setOffset(pagination.getPageIndex());
            orderBy = pagination.getOrderByField();
        }

        int totalCount = 0;
        try {
            totalCount = (int) delegator.findCountByCondition("ProductAndGoodIdentification",
                    EntityCondition.makeCondition(exprs), null, options);
            List<GenericValue> productGIViewList = delegator.findList("ProductAndGoodIdentification",
                    EntityCondition.makeCondition(exprs), null,
            ...
            if (context.containsKey("pagination")) {
                PaginationInputType pagination = (PaginationInputType) context.get("pagination");
                buildConnectionCtx.put("pageIndex", pagination.getPageIndex());
                buildConnectionCtx.put("pageSize", pagination.getPageSize());
            }
    ...
    public static Map<String, Object> searchProductsByName(DispatchContext dctx, Map<String, Object> context) {
        ...
        List<EntityExpr> exprs = UtilMisc.toList(EntityCondition.makeCondition(
                ...
                EntityOperator.AND, EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("productName"),
                        EntityOperator.LIKE, EntityFunction.UPPER(("%" + nameFragment + "%").toUpperCase()))));
        try {
    ...
    public static Map<String, Object> createProduct(DispatchContext dctx, Map<String, Object> context) {
        ...
        if (ServiceUtil.isSuccess(serviceResult)) {
            ...
            try {
                product = EntityQuery.use(delegator).from("Product").where("productId", serviceResult.get("productId"))
                        .cache().queryOne();
